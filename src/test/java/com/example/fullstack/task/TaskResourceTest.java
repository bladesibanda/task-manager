...
public class TaskResourceTest {
  ...
  void updateForbidden() {
    ...
    given()
        ...
        .then()
        .statusCode(401);
    // TODO: TaskService UnauthorizedException should be changed to
    // ForbiddenException
  }
  ...
  void delete() {
    ...
    given()
        ...
        .then()
        .statusCode(204);
    MatcherAssert.assertThat(Task.findById(toDelete.id).await().indefinitely(), nullValue());
  }
  ...
  void setComplete() {
    var toSetComplete = given()
        ...
        .contentType(ContentType.JSON).post("/api/v1/tasks")
        .as(Task.class);
    given().body("\"true\"").contentType(ContentType.JSON)
        .when().put("/api/v1/tasks/" + toSetComplete.id + "/complete")
        .then().statusCode(200);
    MatcherAssert.assertThat(Task.findById(toSetComplete.id).await().indefinitely(),
        allOf(hasProperty("complete", notNullValue()),
